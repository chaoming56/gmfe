...
type MoreSelectDataOptions<T> = MoreSelectNormalDataOptions<T> | MoreSelectGroupDataOptions<T>
...
interface MoreSelectCommonProps<T> {
  ...
  /* 搜索回调 */
  onSearch?(searchWord: string, data: MoreSelectBaseDataOptions<T>[]): Promise<void> | void
  delay?: number
...
interface MoreSelectProps<T> extends MoreSelectCommonProps<T> {
  ...
  onSelect(selected: MoreSelectNormalDataOptions<T> | MoreSelectNormalDataOptions<T>[]): void
}
